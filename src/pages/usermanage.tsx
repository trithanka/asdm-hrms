...
const UserManage: React.FC = () => {
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    fetchData(); // Fetch all users on component mount
  }, []);
  ...
  const paginatedUsers = users.slice(currentPage * rowsPerPage, currentPage * rowsPerPage + rowsPerPage);
  ...
  return (
    ...
    <div style={{ padding: '20px' }}>
       <Toaster />
       <Navbar>
        <Typography variant="h6">User Access Management For HR Dashboard</Typography>
      ...
      <Box display="flex" flexDirection="column" mb={2}>
        ...
        <Box display="flex" justifyContent="flex-start" mb={2}>
          ...
          {selectedFilter && (
            <TextField
              label={`Search by ${selectedFilter === 'loginName' ? 'Login Name' : 'Role ID'}`}
              variant="outlined"
              value={searchTerm}
          ...
          <Button
            ...
            style={{ marginLeft: '20px', width: '130px', height: '45px', marginTop: '5px' }}
          >
            Search
          </Button>
          <Button variant="contained" color="primary" onClick={handleNavigate}  style={{ marginLeft: '20px', width: '130px', height: '45px', marginTop: '5px' }}>
            Dashboard
          </Button>
        </Box>
              ...
              {paginatedUsers.length > 0 ? (
                paginatedUsers.map((user, index) => (
                  <TableRow key={user.loginId}>
                    ...
                    <TableCell style={{ textAlign: 'center' }}>{user.updatedBy}</TableCell>
                    <TableCell style={{ textAlign: 'center' }}>{user.updatedDate}</TableCell>
                    <TableCell style={{ textAlign: 'center' }}>
                      <Button variant="contained" color="primary" style={{ width: '115px' }} onClick={() => handleAccessChange(user.loginId, 0)}>Read</Button>
                      <Button variant="contained" color="secondary" style={{ width: '115px', marginLeft: '10px' }} onClick={() => handleAccessChange(user.loginId, 1)}>Write</Button>
                      <Button variant="contained" color="inherit" style={{ width: '115px', marginLeft: '10px' }} onClick={() => handleAccessChange(user.loginId, "noAccess")}>No Access</Button>
                    </TableCell>
                  </TableRow>
                ))
              ) : (
