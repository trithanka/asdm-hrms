...
export async function useExportEmployeeData(bReleased:any,designation:string,locationName:string) {
  ...
  const response = await employeeDeatils(bReleased, designation, locationName);
    ...
    const rows = response.employeeList?.map((emp: any, index: any) => ({
      ...
      ["Sl.No", "Employee ID", "Name", "Designation", "Phone Number", "Location", "Joining Date"],
    ]);
  
      XLSX.writeFile(workbook, "Total Employees.xlsx", { compression: true });
}

export async function useAttendanceData(staff: any, date: any,dataById?:any) {
    ...
    rows = dataById?.data?.map((emp: any, index: number) => (
      {
        ...
        punchInTime: emp.punchIn === null ? "-- --" : formatTime(emp.punchIn),
        PunchOutTime: emp.punchOut === null ? "-- --" : formatTime(emp.punchOut),
        PunchIn: emp.punchOutOutdoor === 1 ? "Outside" : emp.punchOutOutdoor === 0 ? "Inside" : "-- --",
        attendance_status: emp.punchOutOutdoor === 1 || emp.punchOutdoor === 0 ? "Present" : "Absent",
      }
    ));
  }
      const workbook = XLSX.utils.book_new();
      ...
      XLSX.utils.sheet_add_aoa(worksheet, [
      ["Sl.No", "Attendance Date", "Punch In Time", "Punch Out Time", "Punch In (Outdoor)","Attendance Status"],
      ]);
      }
      XLSX.writeFile(workbook, "Attendace-Report.xlsx",{compression:true})
}

export async function useLeaveReport(type: string, startDate: any, endDate:any) {
  ...
  XLSX.utils.sheet_add_aoa(worksheet, [
    ["Sl.No", "Employee Id", "Name", "Designation", "Type of Leave", "Applied Date", "Start Date", "End Date", "Reason","Location","Last CL Date","Last ML Date","Cl Leave Balance","Ml Leave Balance"],
  ]);
