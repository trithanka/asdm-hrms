...
export default function AttendanceReport({ goPrevious }: any) {
  ...
  function extractEmployeeDetails(records: any): { empId: string | null; empName: string | null; designation: string | null } {
    if (!records || records.length === 0) {
      return { empId: null, empName: null, designation: null };
    }

    const firstRecordWithDetails = records?.find((record: any) => record.empId && record.empName && record.designation)
  ...
  const employeeDetails = extractEmployeeDetails(databyId?.data?.length && databyId.data);
    ...
    <div style={ { width: "100%", display: "flex", flexDirection: "column" } }>
      ...
      <div style={ { display: "flex", gap: 10, alignItems: "center" } }>
        ...
        <LoadingButton
          ...
          onClick={ () => useAttendanceData(staff, startDate, show && databyId) }
        >
          Export excel data
        </LoadingButton>
        ...
        <LoadingButton
          ...
          variant="contained"
          style={ { marginTop: 22 } }
          onClick={ () => generatePDF(targetRef, { filename: "attendance-report.pdf" }) }
        >
          Download PDF
        </LoadingButton>
